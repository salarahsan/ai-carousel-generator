...
# 1. Ab API Key GitHub ki bajaye Streamlit ki secure tijori se aayegi
...
if st.session_state.logged_in:
    ...
    if st.button("Generate Carousel"):
        if st.session_state.credits > 0:
            if topic:
                # Check agar key lagana bhool gaye hain
                if "GROQ_API_KEY" not in os.environ:
                    st.error("⚠️ API Key missing hai! Pehle Streamlit Settings > Secrets mein GROQ_API_KEY dalein.")
                else:
                    with st.spinner("AI Agents are researching and writing... Please wait!"):
                        try:
                            # --- CREWAI AGENTS SETUP (NEW SECURE MODEL) ---
                            researcher = Agent(
                                role='Data Researcher',
                                goal=f'Find 3 key insights about {topic}',
                                backstory='You are an expert at finding viral data points.',
                                llm='groq/llama-3.1-8b-instant', # Yahan humne NAYA model laga diya hai
                                verbose=False
                            )
                            ...
                            writer = Agent(
                                ...
                                llm='groq/llama-3.1-8b-instant', # Yahan bhi NAYA model lag gaya hai
                                verbose=False
                            )
